...
public class VoltDBSend extends Project implements VoltDBRel {
   ...
   public VoltDBSend(
    ...
    @Override public RelOptCost computeSelfCost(RelOptPlanner planner,
        ...
        if (getInput() instanceof VoltDBJoin) {
            rowCount *= 0.2;
        }
    ...
    public double estimateRowCount(RelMetadataQuery mq) {
    	double estimate = getInput().estimateRowCount(mq);
        if (!(getInput() instanceof VoltDBJoin)) {
            estimate *= 10;
        }
        return estimate;
    ...
    public static VoltDBSend convert(RelOptCluster cluster, RelTraitSet traitSet, RelNode child, 
    	    ...
    	    RelDataType rowType,
    	    VoltDBPartitioning partitioning) {
        return new VoltDBSend(cluster, traitSet, child, identities, rowType, partitioning);
       }
    ...
    public RelNode copy(List<RexNode> exprs, RelDataType rowType) {
        ...
        return convert(getCluster(), traitSet, proj, exprs, rowType, m_partitioning);
    }

    @Override
    public Project copy(RelTraitSet traitSet, RelNode input,
            List<RexNode> projects, RelDataType rowType) {
        return convert(getCluster(), traitSet, input, m_identities, rowType, m_partitioning);
    }
